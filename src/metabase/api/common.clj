...
(def ^:private InvokeWithKeepAliveSchema
  {
   ;; Channle that contain any number of newlines followed by the results of the invoked query thunk
   ...
   ;; heartbeat has been sent, this channel is closed and it no longer useful
   :error-channel   (s/protocol async-proto/Channel)
   ;; Future that is invoking the query-thunk. This is mainly useful as testing metadata to see if the future has been
   ;; chancelled or was completed successfully
      ...
      (let [[response-or-timeout c] (async/alts!! [response-chan (async/timeout streaming-response-keep-alive-interval-ms)])]
        (if response-or-timeout
          ...
          (do
            ...
            (async/close! error-chan)
            ;; a newline padding character as it's harmless and will allow us to check if the client is connected. If sending this
            ;; character fails because the connection is closed, the chan will then close.  Newlines are no-ops when
